...
export class Migration20241202080921 extends Migration {
  async up(): Promise<void> {
    ...
    this.addSql(
      'alter table if exists "cart_line_item_adjustment" drop constraint if exists cart_line_item_adjustment_check;'
    )
    this.addSql(
    ...
    this.addSql(
      'alter table if exists "cart_line_item_tax_line" drop column if exists "metadata";'
    )
    this.addSql(
    ...
    this.addSql(
      'alter table if exists "cart_shipping_method" drop constraint if exists cart_shipping_method_check;'
    )
    this.addSql(
  ...
  async down(): Promise<void> {
    ...
    this.addSql(
      'alter table if exists "cart_line_item_adjustment" add constraint cart_line_item_adjustment_check check(amount >= 0);'
    )

    this.addSql(
      'alter table if exists "cart_line_item_tax_line" add column if not exists "metadata" jsonb null;'
    )
    this.addSql(
    ...
    this.addSql(
      'alter table if exists "cart_shipping_method" add constraint cart_shipping_method_check check(amount >= 0);'
    )
