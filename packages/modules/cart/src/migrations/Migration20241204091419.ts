...
export class Migration20241204091419 extends Migration {
  ...
  async up(): Promise<void> {
    ...
    this.addSql('alter table if exists "cart_line_item_adjustment" drop constraint if exists cart_line_item_adjustment_check;');
    ...
    this.addSql('alter table if exists "cart_line_item_tax_line" alter column "rate" type integer using ("rate"::integer);');
    this.addSql('alter table if exists "cart_line_item_tax_line" drop column if exists "metadata";');
    ...
    this.addSql('alter table if exists "cart_shipping_method_tax_line" alter column "rate" type integer using ("rate"::integer);');
  ...
  async down(): Promise<void> {
    ...
    this.addSql('alter table if exists "cart_line_item_adjustment" add constraint cart_line_item_adjustment_check check(amount >= 0);');

    this.addSql('alter table if exists "cart_line_item_tax_line" add column if not exists "metadata" jsonb null;');
    this.addSql('alter table if exists "cart_line_item_tax_line" alter column "rate" type numeric using ("rate"::numeric);');
    ...
    this.addSql('alter table if exists "cart_shipping_method_tax_line" alter column "rate" type numeric using ("rate"::numeric);');
